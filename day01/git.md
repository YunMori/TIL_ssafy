...
git intit = 워킹 파일을 지금부터 git으로 관리하고 싶어 (프로젝트 시작)
...
git restore = 너 지금 작업한거 잘못 작업한거야? 이거 쓰면 이전 버전(이전 commit)으로 돌아감ㅋㅋ
따라서 commit을 안하면 원복도 못합니다....

commit메세지에 어느 파일에 뭘 추가했다 or 고쳤다 맨트 정리해라

# github

repository = 이거 만들기 전에 이미 프로젝트가 이전에 찍힌 버전이 있느냐에 따라 README, gitignore, license 들을 페이지에서 만들까? 파일에서 만들까가 나눠짐.

git remote add origin(별명) (github reop 경로) = git아 내가 이걸 이제 해당 경로로 업로드 할꺼야

git remote -v = 여러 경로 설정시 보는 법
